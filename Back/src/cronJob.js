...
const marcarPublicacionesComoDadasDeBaja = () => {
  ...
  try {
    ...
    mysqlConeccion.query(sql, [fechaLimite], (error, results) => {
      if (error) {
        ...
        // Puedes utilizar este resultado para actualizar las publicaciones y marcarlas como dadas de baja
        // Por ejemplo, puedes recorrer el resultado y actualizar cada publicación individualmente
  ...
  const marcarPublicacionComoDadaDeBaja = (idPublicacion,  mysqlConeccion) => {
      try {
        // Realizar la consulta de actualización para cambiar el estado de alta a baja
...
// Ejecutar cada día a la medianoche (00:00)
cron.schedule('22 21 * * *', () => {
